...
public class PathFollower {
    ...
    public static double headingD = .1;
    public static  double translateP = .1;
    ...
    public PathFollower(HardwareMap hwMap, SampleMecanumDrive drive) {
        this.drive = drive;
        isDone = false;
    ...
    public void goToPoint(Pose2d targetPose, double targetSpeed) {
        ...
        if (!isDone) {
            ...
            if (Math.abs(xError) < 2 && Math.abs(yError) < 2 && Math.abs(headingError) < Math.toRadians(1)) {
                isDone = true;
            }
        } else{
    ...
    public void turnToAbsolute(double degrees, double turnSpeed){
        goToPoint(new Pose2d(drive.getPoseEstimate().getX(), drive.getPoseEstimate().getY(), degrees), turnSpeed);
    }
